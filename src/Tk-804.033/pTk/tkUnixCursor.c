				 ...
				 * for details on legal syntax. */
{
	...
	if (argc == 1) {
	    ...
	    if (XParseColor(display, Tk_Colormap(tkwin), Tcl_GetString(objv[1]),
		    &fg) == 0) {
		Tcl_AppendResult(interp, "invalid color name \"", Tcl_GetString(objv[1]),
			"\"",          NULL);
		goto cleanup;
	    }
	    if (argc == 2) {
		...
		if (XParseColor(display, Tk_Colormap(tkwin), Tcl_GetString(objv[2]),
			&bg) == 0) {
		    Tcl_AppendResult(interp, "invalid color name \"", Tcl_GetString(objv[2]),
			    "\"",          NULL);
		    goto cleanup;
		}
	    }
	...
	if (argc == 2) {
	    ...
	    if (TkReadBitmapFile(interp, display,
		    RootWindowOfScreen(Tk_Screen(tkwin)), Tcl_GetString(objv[1]),
		...
		Tcl_SetResult(interp, "source and mask bitmaps have different sizes", TCL_STATIC);
	    ...
	    if (XParseColor(display, Tk_Colormap(tkwin), Tcl_GetString(objv[3]),
		    &bg) == 0) {
		Tcl_AppendResult(interp, "invalid color name \"", Tcl_GetString(objv[3]),
			"\"",          NULL);
		goto cleanup;
	    }
